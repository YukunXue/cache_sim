...
static void trace_exec(struct trace *t, bool is_check) {
  if (t->t.is_write) {
    ...
    uint32_t ret = cpu_read(t->t.addr, t->t.len, 0);
    //printf("the ret with cache is %d \n", ret);
    if (is_check) {
      uint32_t ret_uncache = cpu_uncache_read(t->t.addr, t->t.len);
      //printf("the ret with uncache is %d \n", ret_uncache);
      if(ret != ret_uncache){
        //printf("the ret with cache is %d \n", ret);
       // printf("the ret with uncache is %d \n", ret_uncache);
        printf("while read with addr: %d && len: %d is error \n", t->t.addr, t->t.len);
        ret = cpu_read(t->t.addr, t->t.len, 1);
      }
...
static void check_diff(void) {
  ...
  for (addr = 0; addr < MEM_SIZE; addr += 4) {
    uint32_t ret = cpu_read(addr, 4, 0);
